 ...
 */
function resolveSendTarget(to) {
    const t = normalizeMaxTarget(to);
    if (MAX_USER_TARGET_RE.test(t))
    ...
    for (let attempt = 1;; attempt++) {
        try {
            return "userId" in target
                ? await bot.api.sendMessageToUser(target.userId, text, extra)
                : await bot.api.sendMessageToChat(target.chatId, text, extra);
        }
        catch (err) {
 ...
 */
export async function sendMaxPairingApproval(bot, id) {
...
export function maskMaxToken(token) {
    if (!token)
        return null;
    if (token.length <= 8)
        return "****";
 ...
 */
export function resolveMaxSendOptions(cfg, channelData) {
    const section = cfg.channels?.[MAX_CHANNEL_ID] ?? {};
    const cd = channelData && typeof channelData === "object" && !Array.isArray(channelData)
        ? channelData
        : {};
    const notify = typeof cd.maxNotify === "boolean"
        ? cd.maxNotify
        : typeof section.notify === "boolean"
            ? section.notify
            : undefined;
    const disableLinkPreview = typeof cd.maxDisableLinkPreview === "boolean"
...
export function initializeBot(token, apiBaseUrl, httpProxy) {
    if (botInstance) {
        try {
...
export function ensureBotForOutbound(cfg) {
    ...
    if (!account.token)
        throw new Error("MAX token is not configured");
...
export async function runPollingLoop(params) {
    ...
    try {
        ...
        if (typeof state.marker === "number")
            marker = state.marker;
        if (state.seenMessageIds?.length)
            primeSeenMessageIds(state.seenMessageIds);
        if (marker != null)
            log?.info?.(`[MAX] polling resumes from persisted marker ${marker}`);
    }
    catch (err) {
    ...
    while (!signal?.aborted) {
        ...
        try {
            const { updates, marker: next } = await bot.api.getUpdates(POLL_ALLOWED_UPDATES, {
                marker,
                limit: 100,
                timeout: 30,
                signal,
