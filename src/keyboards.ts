...
function assertKeyboardLimits(rows: MaxButton[][]): void {
  if (rows.length > MAX_KEYBOARD_LIMITS.maxRows) {
    throw new MaxKeyboardError(
      `inline keyboard has ${rows.length} rows; MAX allows at most ${MAX_KEYBOARD_LIMITS.maxRows}`,
    );
  }
  const total = rows.reduce((sum, row) => sum + row.length, 0);
  if (total > MAX_KEYBOARD_LIMITS.maxButtons) {
    throw new MaxKeyboardError(
      `inline keyboard has ${total} buttons; MAX allows at most ${MAX_KEYBOARD_LIMITS.maxButtons}`,
    );
  }
  rows.forEach((row, rowIdx) => {
    ...
    if (row.length > maxPerRow) {
      throw new MaxKeyboardError(
        `inline keyboard row ${rowIdx + 1} has ${row.length} buttons; MAX allows at most ` +
    ...
    for (const button of row) {
      if (button.type === "link" && button.url.length > MAX_KEYBOARD_LIMITS.maxUrlLength) {
        throw new MaxKeyboardError(
          `link button "${button.text}" URL is ${button.url.length} chars; MAX allows at most ${MAX_KEYBOARD_LIMITS.maxUrlLength}`,
        );
      }
    }
  });
 ...
 */
export function parseInlineKeyboardInput(raw: unknown): MaxButton[][] {
 ...
 */
export function resolveReplyKeyboardButtons(channelData: unknown): MaxButton[][] | null {
  if (!channelData || typeof channelData !== "object" || Array.isArray(channelData)) return null;
  const raw = (channelData as Record<string, unknown>).maxInlineKeyboard;
